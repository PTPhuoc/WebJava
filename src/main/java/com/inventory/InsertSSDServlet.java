...
@WebServlet("/InsertSSD")
@MultipartConfig(
	    fileSizeThreshold = 1024 * 1024, // 1 MB
	    maxFileSize = 1024 * 1024 * 10, // 10 MB
	    maxRequestSize = 1024 * 1024 * 50 // 50 MB
)
public class InsertSSDServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			InsertSSD I = new InsertSSD();
			I.Insert(Id, Name, "Image/" + Id, Type, Double.valueOf(Price) , Producer, Memory, Connect, Speed, MySQLConnect.GetConnect());
